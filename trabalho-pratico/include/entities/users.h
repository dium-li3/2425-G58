...
/**
 * @brief Função que devolve um array de posições de históricos de um dado ano de um utilizador @p u .
 * 
 ...
 * devolve o array de posições de históricos na posição (2024 - @p year )
 * do array de anos.
 * 
 * @param u Utilizador associado aos históricos cujos ids procuramos.
 * @param year Ano ao qual os históricos fazem referencia.
 * 
 * @return Array de posições de históricos referente a um dado ano e a um dado utilizador.
*/
const GArray *get_year_history(User u, int year);

/**
 * @brief Guarda a posição de um histórico no user ao qual ele (o histórico) faz referência.
 * 
 * Adiciona a posição de um histórico no ficheiro de históricos no array de históricos
 * por ano de um User com base no ano em que o histórico aconteceu.
 * 
 ...
 * @param history_pos posição do histórico a guardar no utilizador.
*/
